...
describe('SuggestedActions', () => {
  ...
  it('should render suggested actions container', () => {
    ...
    expect(screen.getByTestId('suggested-actions')).toHaveClass('grid', 'sm:grid-cols-2', 'gap-2', 'w-full');
  });
  ...
  it('should render all four suggested action buttons', () => {
    ...
    expect(screen.getByText("demonstrate djikstra's algorithm")).toBeInTheDocument();
  ...
  it('should call append with correct message when Next.js button is clicked', async () => {
    ...
    const nextjsButton = screen.getByText('What are the advantages').closest('button');
  ...
  it('should call append with correct message when essay button is clicked', async () => {
    ...
    const essayButton = screen.getByText('Help me write an essay').closest('button');
  ...
  it('should call append with correct message when weather button is clicked', async () => {
    ...
    const weatherButton = screen.getByText('What is the weather').closest('button');
  ...
  it('should render button structure with title and label', () => {
    ...
    const titleElements = screen.getAllByText(/^(What are the advantages|Write code to|Help me write an essay|What is the weather)$/);
    const labelElements = screen.getAllByText(/^(of using Next\.js\?|demonstrate djikstra's algorithm|about silicon valley|in San Francisco\?)$/);
  ...
  it('should handle different chat IDs correctly', () => {
    ...
    const firstButton = screen.getByText('What are the advantages').closest('button');
    fireEvent.click(firstButton!);
    
    expect(mockReplaceState).toHaveBeenCalledWith({}, '', '/chat/different-chat-id');
  });
