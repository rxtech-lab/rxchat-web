...
export const modifyNodeTool = (workflow: Workflow) =>
  tool({
    description: 'Modify a node in the workflow',
    parameters: z.object({
      identifier: z.string(),
      node: RegularNodeSchema,
    }),
    execute: async ({ identifier, node }) => {
      workflow.modifyChild(identifier, node);
    },
  });
...
export class Workflow implements WorkflowInterface {
   ...
   */
  private findNode(identifier: string): BaseNode | null {
